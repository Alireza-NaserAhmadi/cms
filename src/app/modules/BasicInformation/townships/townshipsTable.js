  ...
  const removeTownship = (id) => {
    ...
    deleteTownship(user, current.id)
      .then((res) => {
        ...
        if (hasSearched) {
          TownshipFilter(user, searchParams).then((res) => {
            props.getAllTownships(res.data.data);
            setLength(res.data.length);
            setLimit(res.data.length);
          });
    ...
    <>
      ...
      <Card>
        ...
        <CardBody>
          <Filter
            isRtl={true}
            getAllServiceFunction={getAllTownships}
            filterServiceFunction={TownshipFilter}
          ...
          {!length ? null : (
            <ToolkitProvider
              keyField="id"
              data={
                state.townships.length != 0 ? state.townships.townships : []
              }
              columns={columns}
            >
              {(props) => (
        ...
        onSubmit={(values) => {
          ...
          if (!editMode) {
            createTownship(user, values)
              .then((res) => {
                toast.success("کشور با موفقیت اضافه شد");
                if (hasSearched) {
                  TownshipFilter(user, searchParams).then((res) => {
                    props.getAllTownships(res.data.data);
                    setLength(res.data.length);
            ...
            updateTownship(user, current.id, values)
              .then((res) => {
                toast.success("کشور با موفقیت تغییر یافت");
                if (hasSearched) {
                  TownshipFilter(user, searchParams).then((res) => {
                    props.getAllTownships(res.data.data);
                    setLength(res.data.length);
